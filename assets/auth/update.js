...
function submit(event, component){
  ...
  join(component.state.values.toJS());
}

function render(component, updateState) {
  let {props, state} = component;
  if(props.user){
    return <div>Already logged, log out to create another account</div>;
  }
  if(props.pending){
    return <div>User creation pending, please wait</div>;
  }
  return <form onSubmit={e => submit(e, component)}>
    ...
    <div>
      ...
      <input
        ...
        value={state.values.get('email')}
        onChange={e => validate('email', e.target.value, component, updateState)}
        required
      />
      <strong><abbr title="required">*</abbr></strong>
    </div>
